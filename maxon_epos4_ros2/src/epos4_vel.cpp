...
{
public:
  Epos4_Vel_Node(): Node("epos4_cmd_vel_node")
  {
...
private:
  ...
  rclcpp::Client<canopen_interfaces::srv::COTargetDouble>::SharedPtr client_target_;  // this is also target velocity in vel mode
  ...
  void timer_callback()
  {
    ...
    double amplitude = 2000.0; // Maximum value of the sine wave
	...
	if (csv_active == true)
	{
		...
		tpdo_msg.index = 0x60FF;  // Target velocity index
  ...
  void call_trigger_service(rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr client, const std::string& service_name)
  {
      ...
      client->async_send_request(request, std::bind(&Epos4_Vel_Node::trigger_callback, this, std::placeholders::_1));
  }
  ...
  void call_target_vel_service(double target_vel_value)
  {
      ...
      request->target = target_vel_value;
  ...
  void input_loop()
  {
    ...
    while (rclcpp::ok() && !exit)
    {
        ...
        switch (menu_i)
        {
            ...
            case 8:
				csv_active = true;
                sine_wave_time_ = 3.0;  // init position (0) before starting csv mode (when sine_wave_time is 3 then the value is 0)
